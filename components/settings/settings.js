...
function renderThemingSettings() {
    ...
    const currentColors = JSON.parse(localStorage.getItem('theme.colors') || '{}');
...
function renderTabsSyncSettings() {
    ...
    syncToggle.onchange = () => {
        ...
        import('../context-menu/context-menu.js').then(m => {
            if (m.updateSyncTimer) m.updateSyncTimer();
        });
    };
    ...
    manualBtn.onclick = () => {
        import('../context-menu/context-menu.js').then(async m => {
            const changes = await (await import('../tabs/tabs.js')).getSyncChanges();
            m.showSyncModalQueue(changes);
        });
    };

    hardBtn.onclick = async () => {
        const confirmed = await (await import('../modals/modals.js')).showConfirm('Overwrite ALL links with JSON data?');
        if (confirmed) {
            await (await import('../tabs/tabs.js')).syncLinksOverwrite();
